...
Transistor::Transistor(Node* drain, Node* source, Node* gate, double vt, double gm)
    : gate(gate), threshold_voltage(vt), transconductance(gm) {
    n1 = drain;  // n1 -> drain
    n2 = source; // n2 -> source
}

void Transistor::stamp(Matrix& A, std::vector<double>& z, const std::vector<Node*>& nodes, double dt) {
